...
export function PlanningReportsMetricsSection({ summary }: { summary: ReportsSummary }) {
    return (
        <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-6">
            ...
            <MetricCard icon={Activity} label="Saldo líquido" value={formatReportCurrency(summary.net)} toneClassName={summary.net >= 0 ? "text-emerald-200 bg-emerald-500/12" : "text-orange-200 bg-orange-500/12"} />
    ...
    return (
        <article className="rounded-lg border border-white/[0.08] bg-[#111111] p-3">
            <div className="flex items-center justify-between">
                ...
                <span className={`inline-flex h-8 w-8 items-center justify-center rounded-lg ${toneClassName}`}>
                    <Icon size={15} />
                </span>
            </div>
