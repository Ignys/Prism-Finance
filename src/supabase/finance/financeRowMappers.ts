...
export function fromWishItemRow(row: WishItemRow): WishItem {
    return {
        ...
        priority: normalizeWishItemPriority(row.priority),
...
export function toTransactionRow(userId: string, transaction: StoredTransaction): TransactionRow {
    return {
        ...
        payment_for_invoice_id: transaction.paymentForInvoiceId ?? null,
...
export function fromTransactionRow(row: TransactionRow): StoredTransaction {
    return {
        ...
        paymentForInvoiceId: row.payment_for_invoice_id ?? parseInvoicePaymentNote(row.notes)?.invoiceId ?? null,
        notes: row.notes,
        title: row.title,
        categoryId: row.category_id,
        beneficiaryId: row.beneficiary_id,
